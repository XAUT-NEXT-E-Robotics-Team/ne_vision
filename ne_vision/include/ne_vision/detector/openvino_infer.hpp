...
public:
  ...
  OpenvinoInfer(string model_path, string device)
  {
    ...
    // input_shape = { 1, static_cast<unsigned long>(IMAGE_HEIGHT),
    // static_cast<unsigned long>(IMAGE_WIDTH),1};
  ...
  void infer(Mat img, float* detections)
  {
    ...
    copy(output_tensor.data<const float>(),
         output_tensor.data<const float>() + 9,
         detections);
  }
