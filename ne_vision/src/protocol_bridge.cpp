...
bool NePort::init()
{
    ...
    if (ioctl(fd, TCGETS2, &tio) == 0)
    {
        ...
        {
            printf("[WARN] TCSETS2 failed (virtual port?), using default baud rate\n");
        }
    }
    else
    {
        printf("[WARN] TCGETS2 not supported (virtual port?), skipping custom baud rate\n");
    }
...
bool NePort::reopen()
{
    ...
    if (openport() >= 0) return true;
    return false;
}

bool NePort::PortisInit()
{
    return isinit;
}

bool NePort::PortisOpen()
{
    return isopen;
}
