...
OpenvinoInfer::OpenvinoInfer(string model_path_xml,
                             string model_path_bin,
                             string device)
{
  input_shape = {1,
...
void OpenvinoInfer::infer(Mat img, int detect_color)
{
  ...
  //        std::cout <<"timeab: "<< (tb - ta) / cv::getTickFrequency() * 1000
  //        << " "<<std::endl;
  ...
  //        std::cout << "The shape of output tensor:"<<output_shape <<
  //        std::endl;
  // 25200 x 85 Matrix
  const cv::Mat output_buffer(
      output_shape[1], output_shape[2], CV_32F, (void*)output.data<float>());
  ...
  for (int i = 0; i < output_buffer.rows; i++)
  {
    ...
    cv::Mat classes_scores = output_buffer.row(i).colRange(13, 22); // num数字
