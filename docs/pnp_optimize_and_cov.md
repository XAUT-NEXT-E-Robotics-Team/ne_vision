...
在当前优化的场景下，得到协方差矩阵存在两种方法，一是直接从LM中获取最优的雅克比，计算信息矩阵得到协方差。二是使用类似端到端的方式，利用经验建立一个数学模型，并根据数据试图拟合这个模型，或干脆利用查表法来解决这个问题。然而方便起见，我们先采取方案一。
...
![](imgs/cov_yaw_test.png)

yaw轴协方差估计，可以看到，其方差在yaw接近0时较大

![](imgs/cov_x_test.png)

x向（距离向）协方差估计，可以看到，其方差在距离较近，较正对时较小

![](imgs/cov_y_test.png)

y向（横向）协方差估计，可以看到，其方差在距离较近，较正对时较小

![](imgs/cov_x_test_with_x_move.png)

x向（距离向）协方差估计，可以看到，目标移动时，在距离较近时，协方差较小，但在距离较远时，协方差较大
